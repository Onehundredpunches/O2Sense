        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md rounded-3xl p-6 sm:p-8 border border-stone-200/80 dark:border-slate-800 shadow-sm">
            ...
            <div className="mt-8">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-slate-400">
                    (Click vào số tròn trên ảnh để xem chỉ dẫn)
                  </span>
                </div>
                {selectedPinId !== null && (
                  <button
              ...
              <div className="relative rounded-2xl overflow-hidden border-2 border-stone-200/90 dark:border-slate-700/80 shadow-lg bg-slate-950 group">
                <img
                ...
                {currentTopic.hotspots.map((pin) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isSelected && (
                        <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-64 sm:w-72 p-3 bg-slate-900/95 backdrop-blur-md text-white rounded-2xl shadow-2xl border border-teal-500/40 text-xs z-30 pointer-events-none animate-fadeIn">
                          <div className="font-bold text-amber-400 flex items-center gap-1.5 mb-1">
                            <span className="w-4 h-4 rounded-full bg-amber-400 text-slate-950 inline-flex items-center justify-center text-[10px] font-black">
                              {pin.id}
                            </span>
                            <span>{pin.label}</span>
                          </div>
                  ...
                  <div
                    ...
                    className={`p-3 rounded-2xl border transition-all cursor-pointer flex items-start gap-2.5 ${
                      selectedPinId === pin.id
                        ...
                        : 'bg-stone-50 dark:bg-slate-800/40 border-stone-200 dark:border-slate-800/80 hover:border-teal-500/40 text-slate-700 dark:text-slate-300'
                    }`}
                  >
                    <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[11px] font-black flex-shrink-0 mt-0.5 ${
                      selectedPinId === pin.id ? 'bg-amber-400 text-slate-950' : 'bg-teal-600 text-white'
                    }`}>
                      {pin.id}
                    </span>
